...
const EventModal = (props) => {
  ...
  const todayFormat = `${todayYear}-${String(todayMonth).padStart(
    2,
    "0"
  ...
  useEffect(() => {
    ...
    if (id !== null) {
      ...
      const targetDateFormat = `${targetStartYear}-${String(
        targetStartMonth + 1
      ).padStart(2, "0")}-${String(targetStartDay).padStart(2, "0")}`;
      const targetStartTime = `${String(targetStartHours).padStart(
        2,
        "0"
      )}:${String(targetStartMinutes).padStart(2, "0")}`;

      // 終了日付
      const targetEndHours = targetEndDate.getHours();
      const targetEndMinutes = targetEndDate.getMinutes();
      const targetEndTime = `${String(targetEndHours).padStart(
        2,
        "0"
      )}:${String(targetEndMinutes).padStart(2, "0")}`;
  ...
  return (
    <Modal show={props.modalFlg} onHide={handleClose} centered>
      <Modal.Header>
        <Modal.Title>
          <h5>
            {props.targetEvent.id === null ? "パンの予約" : "予定の編集・削除"}
          </h5>
        </Modal.Title>
      </Modal.Header>
      <Modal.Body>
